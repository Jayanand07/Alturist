    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium">Full CRUD operations and subscription management.</p>
        </div>
      </div>
      ...
      <Card className="border-none shadow-sm bg-white overflow-hidden">
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-8 space-y-4">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-2xl" />)}</div>
          ) : patients.length === 0 ? (
             <div className="flex flex-col items-center justify-center p-20 text-center space-y-6">
                <div className="bg-gray-50 p-8 rounded-full text-gray-300"><FilterX size={64} /></div>
              ...
              <Table>
                ...
                <TableBody>
                  {patients.map((patient: Patient) => (
                    <TableRow key={patient.id} className="border-gray-50 hover:bg-blue-50/20 transition-colors group">
                      <TableCell className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col min-w-0">
                            <span className="text-sm font-bold text-gray-900 truncate">{patient.fullName}</span>
                      ...
                      <TableCell>
                         <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-gray-400 font-bold">{patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString('en-IN') : "N/A"}</span>
                         </div>
                      </TableCell>
                      <TableCell className="text-right pr-6">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="sm" className="h-8 text-[10px] font-bold text-gray-500 hover:text-amber-600 hover:bg-amber-50 rounded-xl" onClick={() => { setSelectedPatientId(patient.id); setIsSubOpen(true); }}>Manage Sub</Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-xl" onClick={() => { setSelectedPatientId(patient.id); setIsDetailsOpen(true); }}><Eye size={16} /></Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl" onClick={() => handleEditClick(patient)}><Edit2 size={16} /></Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl" onClick={() => { setPatientToDelete(patient); setIsDeleteDialogOpen(true); }}><Trash2 size={16} /></Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-3xl p-0 overflow-y-auto max-h-[90vh] border-none rounded-[32px] shadow-2xl">
          {isLoadingDetails ? (
            ...
            <div className="flex flex-col bg-white">
              <div className="bg-gradient-to-br from-blue-600 to-teal-600 p-8 text-white relative">
                <div className="flex items-center gap-6">
                  <Avatar className="h-20 w-20 border-4 border-white/20 shadow-2xl">
                    <AvatarFallback className="bg-white text-blue-600 font-black text-2xl">{getInitials(details.fullName || details.email || '')}</AvatarFallback>
                  </Avatar>
                  <div>
              ...
              <div className="p-8 space-y-6">
                 <div>
                    <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider mb-2">Health Information</h4>
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 bg-gray-50 rounded-2xl">
                          <p className="text-[10px] font-black text-gray-400 uppercase">Allergies</p>
                          <p className="text-sm font-bold text-gray-800 mt-1">{details.allergies || "None reported"}</p>
                       </div>
                       <div className="p-4 bg-gray-50 rounded-2xl">
                          <p className="text-[10px] font-black text-gray-400 uppercase">Chronic Conditions</p>
                          <p className="text-sm font-bold text-gray-800 mt-1">{details.chronicConditions || "None reported"}</p>
                       </div>
                    </div>
                 </div>
                 
                 <div>
                    <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider mb-2">Subscription Status</h4>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-xl p-0 overflow-hidden border-none rounded-[32px] shadow-2xl">
           <div className="bg-blue-600 p-8 text-white relative">
             <DialogTitle className="text-2xl font-black">Edit Patient Profile</DialogTitle>
           </div>
           <form onSubmit={handleEditSubmit} className="p-8 space-y-5 bg-white">
             {isLoadingDetails ? <Loader2 className="animate-spin mx-auto text-blue-600" /> : (
               <div className="grid grid-cols-2 gap-5">
                 <div className="space-y-2 col-span-2">
                    ...
                    <Input required placeholder="Patient Name" className="h-11 rounded-xl" value={editForm.fullName} onChange={e => setEditForm({...editForm, fullName: e.target.value})} />
                 </div>
                 <div className="space-y-2">
                    <Label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Phone</Label>
                    <Input placeholder="+91 9876543210" className="h-11 rounded-xl" value={editForm.phone} onChange={e => setEditForm({...editForm, phone: e.target.value})} />
                 </div>
                 <div className="space-y-2">
                    <Label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">City</Label>
                    <Input placeholder="Delhi" className="h-11 rounded-xl" value={editForm.city} onChange={e => setEditForm({...editForm, city: e.target.value})} />
                 </div>
                 <div className="space-y-2">
                    <Label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Blood Group</Label>
                    <Input placeholder="O+" className="h-11 rounded-xl" value={editForm.bloodGroup} onChange={e => setEditForm({...editForm, bloodGroup: e.target.value})} />
                 </div>
                 <div className="space-y-2 col-span-2">
                    <Label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Allergies</Label>
                    <Textarea className="rounded-xl" value={editForm.allergies} onChange={e => setEditForm({...editForm, allergies: e.target.value})} />
                 </div>
                 <div className="space-y-2 col-span-2">
                    <Label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Chronic Conditions</Label>
             ...
             <DialogFooter className="pt-4 gap-3">
                <Button type="button" variant="ghost" onClick={() => setIsEditOpen(false)} className="rounded-xl font-bold h-11">Cancel</Button>
                <Button type="submit" disabled={updateMutation.isPending} className="rounded-xl font-bold px-8 h-11 bg-blue-600 hover:bg-blue-700 text-white">
      ...
      <Dialog open={isSubOpen} onOpenChange={setIsSubOpen}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none rounded-[32px] shadow-2xl">
           <div className="bg-amber-500 p-8 text-white relative">
             <DialogTitle className="text-2xl font-black">Manage Subscription</DialogTitle>
           </div>
                     ...
                     {details?.subscription?.status === 'ACTIVE' ? (
                        <div>
                           <div className="flex justify-between items-center mb-4">
                              <span className="font-bold text-gray-900">{details.subscription.planName} ({details.subscription.billingCycle})</span>
                        ...
                        <p className="text-sm font-bold text-gray-600">No active subscription found for this user.</p>
                     )}
                  </div>
                  
                  <form onSubmit={handleAssignSub} className="space-y-4 pt-4 border-t border-gray-100">
                     <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider">Assign New Plan</h4>
                     <div className="space-y-2">
                        <Label className="text-xs font-black text-gray-400 uppercase px-1">Select Plan</Label>
                        <Select value={subForm.planId} onValueChange={v => setSubForm({...subForm, planId: v})}>
                           <SelectTrigger className="h-11 rounded-xl"><SelectValue placeholder="Choose Plan" /></SelectTrigger>
                           <SelectContent>
                     ...
                     <div className="space-y-2">
                        <Label className="text-xs font-black text-gray-400 uppercase px-1">Billing Cycle</Label>
                        <Select value={subForm.billingCycle} onValueChange={v => setSubForm({...subForm, billingCycle: v})}>
                           <SelectTrigger className="h-11 rounded-xl"><SelectValue placeholder="Billing Cycle" /></SelectTrigger>
                           <SelectContent>
                     ...
                     <Button type="submit" disabled={assignSubMutation.isPending} className="w-full rounded-xl font-bold bg-amber-500 hover:bg-amber-600 text-white h-11 shadow-lg shadow-amber-500/20">
